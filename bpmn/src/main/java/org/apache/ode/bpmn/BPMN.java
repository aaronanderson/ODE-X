...
import org.apache.ode.bpmn.compiler.DiscoveryPass;
import org.apache.ode.bpmn.compiler.EmitPass;
import org.apache.ode.bpmn.compiler.parser.DefinitionsParser;
...
public class BPMN {
	...
	public static final String BPMN_MIMETYPE = "application/bpmn";
	...
	private static final Logger log = Logger.getLogger(BPMN.class.getName());
	...
	@Inject
	Provider<BPMNContext> ctxProvider;
	@Inject
	...
	@PostConstruct
	public void init() {
		log.fine("Initializing BPMNPlugin");
		repository.registerFileExtension("bpmn", BPMN_MIMETYPE);
		repository.registerNamespace(BPMN_NAMESPACE, BPMN_MIMETYPE);
		xmlValidate.registerSchemaSource(BPMN_MIMETYPE, new SchemaSource() {
		...
		repository.registerCommandInfo(BPMN_MIMETYPE, Validate.VALIDATE_CMD, true, xmlValidate.getProvider());
		repository.registerHandler(BPMN_MIMETYPE, new XMLDataContentHandler() {
		...
		platform.registerComponent(bpmnComponent);
		...
		ExecCompiler bpmnCompiler = new ExecCompiler();
		bpmnCompiler.addInstructionSet(BPMNComponent.BPMN_INSTRUCTION_SET);
		bpmnCompiler.addInstructionSet(WSDLComponent.WSDL_INSTRUCTION_SET);
		bpmnCompiler.addSubContext(XSDContext.ID, schemaProvider);
		bpmnCompiler.addSubContext(WSDLContext.ID, wsdlProvider);
		bpmnCompiler.addSubContext(BPMNContext.ID, ctxProvider);
		bpmnCompiler.addCompilerPass(CompilerPhase.INITIALIZE, new DiscoveryPass());
		bpmnCompiler.addCompilerPass(CompilerPhase.DISCOVERY, new DiscoveryPass());
		bpmnCompiler.addCompilerPass(CompilerPhase.EMIT, new EmitPass());
		...
		// DiscoveryPass());
		ParserRegistry preg = bpmnCompiler.handlerRegistry(ParserRegistry.class);
		try {
			preg.register(new DefinitionsParser(), DefinitionsParser.DEFINITIONS);
			
		} catch (ParserException pe) {
			log.log(Level.SEVERE, "", pe);
		}
		compilers.register(bpmnCompiler, BPMN_MIMETYPE);

	
		log.fine("BPMNPlugin Initialized");
