 ...
 * under the License.
 */
package org.apache.ode.bpmn.compiler;
...
public class DiscoveryPass implements CompilerPass<ExecCompilerContext> {
	...
	public void compile(ExecCompilerContext ctx) {
		BPMNContext bctx = ctx.subContext(BPMNContext.ID, BPMNContext.class);
		bctx.getClass();
		switch (ctx.phase()) {
		case DISCOVERY:
			try {
				...
				while (reader.hasNext()) {
					...
					case XMLStreamConstants.START_ELEMENT:
						if (DefinitionsParser.DEFINITIONS.equals(reader.getName())) {
